...
namespace Remotely.Agent.Installer.Win.Services
{
    public class InstallerService
    {
        ...
        /** Return device ID if install success, otherwise null */
        public async Task<string> Install(string serverUrl,
            ...
            bool createSupportShortcut)
        {
            try
            {
                ...
                FileIO.Copy(Assembly.GetExecutingAssembly().Location, Path.Combine(_installPath, "Tuso.exe"));
                ...
                return connectionInfo.DeviceID;
            }
            catch (Exception ex)
            {
                Logger.Write(ex);
        ...
        private void CreateSupportShortcut(string serverUrl, string deviceUuid, bool createSupportShortcut)
        {
            ...
            shortcut.IconLocation = Path.Combine(_installPath, "Tuso_Agent.exe");
        ...
        {
            var version = FileVersionInfo.GetVersionInfo(Path.Combine(_installPath, "Tuso_Agent.exe"));
            ...
            remotelyKey.SetValue("DisplayIcon", Path.Combine(_installPath, "Tuso_Agent.exe"));
            remotelyKey.SetValue("DisplayName", "Tuso");
            remotelyKey.SetValue("DisplayVersion", version.FileVersion);
            remotelyKey.SetValue("InstallDate", DateTime.Now.ToShortDateString());
            remotelyKey.SetValue("Publisher", "Tuso");
            ...
            remotelyKey.SetValue("UninstallString", Path.Combine(_installPath, "Tuso.exe -uninstall -quiet"));
            remotelyKey.SetValue("QuietUninstallString", Path.Combine(_installPath, "Tuso.exe -uninstall -quiet"));
        }
        ...
        private void InstallService()
        {
            ...
            if (serv == null)
            {
                var command = new string[] { "/assemblypath=" + Path.Combine(_installPath, "Tuso_Agent.exe") };
