...
Copy-Item -Path "$Root\Agent.Installer.Win\bin\Release\Tuso.exe" -Destination "$Root\Server\wwwroot\Content\Tuso.exe" -Force
Add-DataBlock -FilePath "$Root\Server\wwwroot\Content\Tuso.exe"

if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /fd SHA256 /f "$CertificatePath" /p $CertificatePassword /t http://timestamp.digicert.com "$Root\Server\wwwroot\Content\Tuso.exe"
}
